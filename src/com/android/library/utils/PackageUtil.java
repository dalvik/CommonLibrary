...
import com.android.library.utils.ShellUtils.CommandResult;
...
public class PackageUtil {
    ...
    /**
     * Uninstall return code<br/>
     * uninstall fail if the system failed to delete the package because it is the
     ...
     * @return
     */
    public static int getPackageSignatureHashcode(Context ctx, String gamePkgName) {
        int signatureHashCode = 0;
        PackageInfo pkgInfo;
        PackageManager pManager = ctx.getPackageManager();

        try {
            pkgInfo = pManager.getPackageInfo(gamePkgName, PackageManager.GET_SIGNATURES);
     ...
     * @return
     */
    public static final synchronized int install(Context context, String filePath) {
     ...
     */
    public static synchronized boolean installNormal(Context context, String filePath) {
     ...
     * <li>You should add <strong>android.permission.INSTALL_PACKAGES</strong> in
     * manifest, so no need to request root permission, if you are system app.</li>
     * </ul>
     *
     * @param context  file path of package
     * @param filePath file path of package
     * @return {@link PackageUtil#INSTALL_SUCCEEDED} means install success, other
     * means failed. details see {@link PackageUtils#INSTALL_FAILED_*}
     */
    private static synchronized int installSilent(Context context, String filePath) {
         ...
         * <uses-permission android:name="android.permission.INSTALL_PACKAGES" /> in
         * mainfest
         **/
        ...
        if (commandResult.successMsg != null
            ...
            commandResult = ShellUtils.execCommand(command.toString(), isSystemApplication(context), true);
            if (commandResult.successMsg != null
                    && (commandResult.successMsg.contains("Success") || commandResult.successMsg.contains("success"))) {
                return INSTALL_SUCCEEDED;
            }
        ...
        Log.e(
                TAG,
                new StringBuilder().append("successMsg:").append(commandResult.successMsg).append(", ErrorMsg:")
                        .append(commandResult.errorMsg).toString());
     ...
     * @return
     */
    public static final synchronized int uninstall(Context context, String packageName) {
        int result = 0;
        if (!isRoot) {
            isRoot = ShellUtils.checkRootPermission();
        }

        if (PackageUtil.isSystemApplication(context) || isRoot) {
     ...
     */
    public static synchronized boolean uninstallNormal(Context context, String packageName) {
        if (packageName == null || packageName.length() == 0) {
            return false;
        }

        Intent i = new Intent(Intent.ACTION_DELETE, Uri.parse(new StringBuilder(64).append("package:").append(packageName)
                .toString()));
        i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        context.startActivity(i);
        return true;
     ...
     */
    public static synchronized int uninstallSilent(Context context, String packageName) {
     ...
     * @param context     file path of package
     * @param packageName package name of app
     * @param isKeepData  whether keep the data and cache directories around after package
     *                    removal
     * @return <ul>
     ...
     * denied</li>
     */
    public static synchronized int uninstallSilent(Context context, String packageName, boolean isKeepData) {
        if (packageName == null || packageName.length() == 0) {
        ...
        /**
         * if context is system app, don't need root permission, but should add
         * <uses-permission android:name="android.permission.DELETE_PACKAGES" /> in
         * mainfest
         **/
